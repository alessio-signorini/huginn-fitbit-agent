module Agents
  class FitbitAgent < Agent
    ...
    description <<-MD
      ...
      It will remember some (`loopback`=50) activities so it can update them
      ...
      Follow [these instructions](https://dev.fitbit.com/apps/oauthinteractivetutorial?clientEncodedId=YOURCLIENT&clientSecret=YOURSECRET&redirectUri=https://localhost:3000/fitbit/callback&applicationType=PERSONAL)
    ...
    def validate_options
      ...
      unless options['oauth2_client'].present? && interpolated['oauth2_client'].match(/^[A-Z0-9]+$/)
        errors.add(:base, ":oauth2_client needs to be defined")
      end

      unless options['oauth2_secret'].present? && interpolated['oauth2_secret'].match(/^\w+$/)
        errors.add(:base, ":oauth2_secret needs to be defined")
      end
    end
...
#    end
    ...
    def oauth2_client
      OAuth2::Client.new(interpolated['oauth2_client'],interpolated['oauth2_secret'],
